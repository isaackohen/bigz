				...
				<tr class="first"><th>
					...
					<tr class="second"><th>
						...
						<tr class="third"><th>
							...
							<th>
								@if($currencystated != 'usd')
								<div>{{ number_format(floatval($entry->wager), 8, '.', '') }} <img src="/img/currency/svg/{{ \App\Currency\Currency::find($currencystated)->icon() }}.svg" style="width: 14px; height: 14px;"></div>
								@else
								<div>≈ {{ number_format(floatval($entry->usd_wager), 2, '.', '') }} <img src="/img/currency/svg/usd.svg" style="width: 14px; height: 14px;"></div>
								@endif
							</th>
							<th>
								@if($currencystated != 'usd')
								<div>{{ number_format(floatval($entry->profit), 8, '.', '') }} <img src="/img/currency/svg/{{ \App\Currency\Currency::find($currencystated)->icon() }}.svg" style="width: 14px; height: 14px;"></div>
								@else
								<div>≈ {{ number_format(floatval($entry->usd_profit), 2, '.', '') }} <img src="/img/currency/svg/usd.svg" style="width: 14px; height: 14px;"></div>
								@endif
							</th>
						</tr>
						@endforeach
 ...
 <button data-mdb-ripple-radius="5" class="btn btn-transparent">
		<div style="width: 100px !important;display: flex; font-size: 10px; justify-content: center;margin-left: auto;margin-right: auto;">
			<select id="currency-selector-leader" style="min-width: 100px;">
				...
				<option value="{{ $currency->id() }}" data-icon="{{ $currency->icon() }}">
					{{ $currency->name() }}
				</option>
				@endforeach
			</select>
