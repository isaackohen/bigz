...
class Leaderboard extends Model
{
    ...
    public static function insert(Game $game) {
        ...
        self::insertGame('hourly', $game);
        self::insertGame('daily', $game);
		self::insertGame('weekly', $game);
    }

    private static function insertGame($type, Game $game) {
        $currency = Currency::find($game->currency);

        $entry = Leaderboard::where('type', $type)->where('currency', $currency->id())->where('user', $game->user)->where('time', self::toTime($type))->first();
    ...
    public static function getLeaderboardByCurrency($positions, string $type, \App\Currency\Currency $currency, string $orderBy = 'wager'): array {
        $result = [];
		if(Leaderboard::where('type', $type)->first() == null) return reject(2, 'Invalid type');
        foreach(Leaderboard::where('type', $type)->where('currency', $currency->id())->where('time', self::toTime($type))->orderBy($orderBy, 'desc')->take($positions)->get() as $entry) {
            array_push($result, [
                'entry' => $entry->toArray(),
    ...
    private static function toTime($type) {
        switch ($type) {
            case 'hourly': $mark = Carbon::now()->minute(59)->second(59)->timestamp; break;
            case 'daily': $mark = Carbon::now()->endOfDay()->timestamp; break;
			case 'weekly': $mark = Carbon::now()->endOfWeek()->timestamp; break;
