...
class User extends \Jenssegers\Mongodb\Auth\User implements JWTSubject {
    ...
    protected $fillable = [
        ...
        'referral', 'referral_wager_obtained', 'referral_bonus_obtained', 'promocode_limit_reset', 'promocode_limit', 'tg_linked', 'firstdepo',
        ...
        'wallet_trx_private_key', 'freegames', 'freegames_balance', 'referral_balance_usd', 'bonus1', 'bonus1_goal', 'bonus1_currency', 'bonus1_wager', 'bonus2', 'bonus2_goal', 'bonus2_currency', 'bonus2_wager'
    ];
    ...
    protected $hidden = [
        ...
        'demo_btc', 'demo_ltc', 'demo_eth', 'demo_doge', 'demo_bch', 'demo_trx', 'demo_xrp', 'demo_usdt', 'demo_usdc', 'demo_bnb', 'demo_bonus',
        'wallet_btc', 'wallet_ltc', 'wallet_eth', 'wallet_doge', 'wallet_bch', 'wallet_trx', 'wallet_xrp', 'wallet_usdt', 'wallet_usdc', 'wallet_bnb', 'wallet_bonus',
        'wallet_trx_private_key', 'bonus1', 'bonus1_wager', 'bonus1_currency', 'bonus1_wager', 'bonus2', 'bonus2_goal', 'bonus2_currency', 'bonus2_wager'
    ];
    ...
    */
    ...
    public function pokerbalance(): float {
            $api = new PokerApi(541, 'lkq9b2br-w8oy-01oy-qgtv-48a09v8sz91a', '217.182.195.96', 4000);
            $api->connect();
            $login = $this->name;
            $getid = $api->getIdByLogin($login);

            $getBalance = $api->getBalance($getid);

        $getBalance = $getBalance['aogetbalanceinfo_2']['account'][1]['@attributes']['balance'];



        Log::notice($getBalance);

        return round($getBalance, 2);
    }

    public function telegramLinked() {
